  ...
  namespace chain {
    ...
    clean_database_fixture::clean_database_fixture ( )
    {
      try {
        ...
        for ( int i = 1; i < argc; i++ ) {
          ...
          if ( arg == "--show-test-names" )
            std::cout << "running test "
                      << boost::unit_test::framework::current_test_case ( ).p_name << std::endl;
        }
        ...
        for ( int i = STEEMIT_NUM_INIT_MINERS; i < STEEMIT_MAX_WITNESSES; i++ ) {
          ...
          fund ( STEEMIT_INIT_MINER_NAME + fc::to_string ( i ),
                 STEEMIT_MIN_PRODUCER_REWARD.amount.value );
          witness_create ( STEEMIT_INIT_MINER_NAME + fc::to_string ( i ), init_account_priv_key,
                           "foo.bar", init_account_pub_key, STEEMIT_MIN_PRODUCER_REWARD.amount );
        }
    ...
    clean_database_fixture::~clean_database_fixture ( )
    {
      try {
        ...
        // This way, boost test's last checkpoint tells us approximately where the
        // error was.
    ...
    live_database_fixture::live_database_fixture ( )
    {
      try {
        ...
        FC_ASSERT ( fc::exists ( _chain_dir ),
                    "Requires blockchain to test on in ./test_blockchain" );
    ...
    void database_fixture::generate_block ( uint32_t skip, const fc::ecc::private_key& key,
                                            int miss_blocks )
    {
      skip |= default_skip;
      db_plugin->debug_generate_blocks ( graphene::utilities::key_to_wif ( key ), 1, skip,
                                         miss_blocks );
    }
    ...
    const account_object& database_fixture::account_create (
      const string& name, const string& creator, const private_key_type& creator_key,
                                                             ...
                                                             const public_key_type& post_key )
    {
      try {
        return account_create (
                 name, STEEMIT_INIT_MINER_NAME, init_account_priv_key,
        ...
        [ = ]( database & db ) {
          ...
          if ( amount.symbol == SBD_SYMBOL ) {
            ...
            if ( median_feed.current_median_history.is_null ( ) )
              ...
              f.current_median_history =
                price ( asset ( 1, SBD_SYMBOL ), asset ( 1, STEEM_SYMBOL ) );
            } );
          }
    ...
    void database_fixture::transfer ( const string& from, const string& to,
    ...
    void database_fixture::vest ( const string& account, const asset& amount )
    {
      ...
      [ = ]( database & db ) {
        db.modify (
          db.get_dynamic_global_properties ( ),
    ...
    vector< operation > database_fixture::get_last_operations ( uint32_t num_ops )
    {
      ...
      const auto& acc_hist_idx =
        db.get_index< account_history_index > ( ).indices ( ).get< by_id > ( );
      auto itr = acc_hist_idx.end ( );

      while ( itr != acc_hist_idx.begin ( ) && ops.size ( ) < num_ops ) {
        itr--;
        ops.push_back (
          fc::raw::unpack< steemit::chain::operation > ( db.get ( itr->op ).serialized_op ) );
      }
    ...
    namespace test {
      ...
      void _push_transaction ( database& db, const signed_transaction& tx,
