...
namespace steemit {
  namespace account_by_key {
    ...
    namespace detail {
      ...
      vector< vector< account_name_type > > account_by_key_api_impl::get_key_references(
        vector< public_key_type >& keys )const
      {
        ...
        const auto& key_idx =
          _app.chain_database()->get_index< key_lookup_index >().indices().get< by_key >();
    ...
    vector< vector< account_name_type > > account_by_key_api::get_key_references(
      vector< public_key_type > keys )const
    {
