  ...
  namespace chain {
    ...
    inline void validate_permlink_0_1 ( const string& permlink )
    {
      FC_ASSERT ( permlink.size() > STEEMIT_MIN_PERMLINK_LENGTH &&
                  permlink.size() < STEEMIT_MAX_PERMLINK_LENGTH, "Permlink is not a valid size." );
    ...
    void account_create_evaluator::do_apply ( const account_create_operation& o )
    {
      ...
      FC_ASSERT ( creator.balance >= o.fee, "Insufficient balance to create account.",
                  ( "creator.balance",
                    creator.balance ) ( "required", o.fee ) );
      //! at calibrae genesis has_hardfork always evaluates true
      ...
      FC_ASSERT ( o.fee >= asset ( wso.median_props.account_creation_fee.amount *
                  ...
                  ( "f", wso.median_props.account_creation_fee * asset ( STEEMIT_CREATE_ACCOUNT_WITH_STEEM_MODIFIER,
                                                                         STEEM_SYMBOL ) )
                  ( "p", o.fee ) );
      //! first condition is true so second is irrelevant
    ...
    void account_create_with_delegation_evaluator::do_apply ( const
                                                              account_create_with_delegation_operation& o )
    {
      //! has_hardfork always evaluates true at calibrae genesis
      ...
      FC_ASSERT ( creator.balance >= o.fee, "Insufficient balance to create account.",
                  ...
                  ( "required", o.fee ) );
      FC_ASSERT ( creator.vesting_shares - creator.delegated_vesting_shares - asset (
                    creator.to_withdraw - creator.withdrawn, VESTS_SYMBOL ) >= o.delegation,
                  ...
                  ( "creator.delegated_vesting_shares", creator.delegated_vesting_shares ) ( "required",
                                                                                             o.delegation ) );
      auto target_delegation = asset ( wso.median_props.account_creation_fee.amount *
                                       STEEMIT_CREATE_ACCOUNT_WITH_STEEM_MODIFIER * STEEMIT_CREATE_ACCOUNT_DELEGATION_RATIO,
                                       STEEM_SYMBOL ) * props.get_vesting_share_price();
      auto current_delegation = asset ( o.fee.amount * STEEMIT_CREATE_ACCOUNT_DELEGATION_RATIO,
                                        STEEM_SYMBOL ) * props.get_vesting_share_price() + o.delegation;
    ...
    void delete_comment_evaluator::do_apply ( const delete_comment_operation& o )
    {
      ...
      if (
        //! at calibrae genesis has_hardfork always evaluates true
        //   _db.has_hardfork( STEEMIT_HARDFORK_0_6__80 ) &&
    ...
    struct comment_options_extension_visitor {
      comment_options_extension_visitor ( const comment_object& c, database& db ) : _c ( c ),
        _db ( db ) {}
      ...
      try {
        ...
        if ( o.parent_author != STEEMIT_ROOT_POST_PARENT ) {
          ...
          FC_ASSERT ( parent->depth < STEEMIT_MAX_COMMENT_DEPTH,
                      "Comment is nested ${x} posts deep, maximum depth is ${y}.", ( "x", parent->depth ) ( "y",
        ...
        //! at calibrae genesis has_hardfork always evaluates true
        if (
          // ( _db.has_hardfork( STEEMIT_HARDFORK_0_17__926 ) ) &&
        ...
        if ( itr == by_permlink_idx.end() ) {
          ...
          if ( o.parent_author == STEEMIT_ROOT_POST_PARENT ) {
            FC_ASSERT ( ( now - auth.last_root_post ) > STEEMIT_MIN_ROOT_COMMENT_INTERVAL,
                        "You may only post once every 5 minutes.", ( "now", now ) ( "last_root_post",
                                                                                    auth.last_root_post ) );
          }
          ...
          //     { FC_ASSERT( (now - auth.last_post) > STEEMIT_MIN_REPLY_INTERVAL, "You may only comment once every 20 seconds.", ("now", now)("auth.last_post", auth.last_post) ); }
          //   } else {
          //     FC_ASSERT( (now - auth.last_post) > fc::seconds(60), "You may only post once per minute.", ("now",
          //                                                                                                 now)("auth.last_post", auth.last_post) );
          //   }
          ...
          //! at calibrae genesis has_hardfork always evaluates true
          if (
            //   _db.has_hardfork( STEEMIT_HARDFORK_0_12__176 ) &&
            //    !_db.has_hardfork( STEEMIT_HARDFORK_0_17__733 ) &&
            o.parent_author == STEEMIT_ROOT_POST_PARENT ) {
            uint64_t post_delta_time = std::min ( _db.head_block_time().sec_since_epoch() -
                                                  auth.last_root_post.sec_since_epoch(), STEEMIT_POST_AVERAGE_WINDOW );
            uint32_t old_weight = uint32_t ( ( post_bandwidth * ( STEEMIT_POST_AVERAGE_WINDOW -
                                                                  post_delta_time ) ) / STEEMIT_POST_AVERAGE_WINDOW );
            ...
            reward_weight = uint16_t ( std::min ( ( STEEMIT_POST_WEIGHT_CONSTANT * STEEMIT_100_PERCENT ) /
                                                  ( post_bandwidth * post_bandwidth ), uint64_t ( STEEMIT_100_PERCENT ) ) );
          }
          ...
          const auto& new_comment = _db.create< comment_object > ( [&] ( comment_object & com ) {
            ...
            //! This shining example of badly written code executes one of two complex
            //! strings of code, when if the condition in the block directly below it
            //! would not require the first two blocks to be executed post HF17
            // if ( o.parent_author == STEEMIT_ROOT_POST_PARENT ) {
            //   com.parent_author = "";
          ...
          //! at calibrae genesis has_hardfork always evaluates true
          //! any negated has_hardfork automatically is false because genesis is the HF19 state
          //! negated has_hardfork means 'before' and genesis is after the newest steem HF
    ...
    void escrow_transfer_evaluator::do_apply ( const escrow_transfer_operation& o )
    {
      try {
        ...
        FC_ASSERT ( from_account.balance >= steem_spent,
                    "Account cannot cover STEEM costs of escrow. Required: ${r} Available: ${a}", ( "r",
                                                                                                    steem_spent ) ( "a", from_account.balance ) );
        FC_ASSERT ( from_account.sbd_balance >= sbd_spent,
                    "Account cannot cover SBD costs of escrow. Required: ${r} Available: ${a}", ( "r",
                                                                                                  sbd_spent ) ( "a",
                                                                                                      from_account.sbd_balance ) );
    ...
    void escrow_approve_evaluator::do_apply ( const escrow_approve_operation& o )
    {
      try {
        ...
        FC_ASSERT ( escrow.to == o.to, "Operation 'to' (${o}) does not match escrow 'to' (${e}).", ( "o",
                                                                                                     o.to ) ( "e", escrow.to ) );
        FC_ASSERT ( escrow.agent == o.agent,
                    "Operation 'agent' (${a}) does not match escrow 'agent' (${e}).", ( "o", o.agent ) ( "e",
                                                                                                         escrow.agent ) );
        ...
        if ( o.who == o.to ) {
          FC_ASSERT ( !escrow.to_approved, "Account 'to' (${t}) has already approved the escrow.", ( "t",
                                                                                                     o.to ) );
        ...
        if ( reject_escrow ) {
          ...
          if ( escrow.to_approved && escrow.agent_approved ) {
    ...
    void escrow_dispute_evaluator::do_apply ( const escrow_dispute_operation& o )
    {
      try {
        ...
        FC_ASSERT ( e.to == o.to, "Operation 'to' (${o}) does not match escrow 'to' (${e}).", ( "o",
                                                                                                o.to ) ( "e", e.to ) );
    ...
    void escrow_release_evaluator::do_apply ( const escrow_release_operation& o )
    {
      try {
        ...
        FC_ASSERT ( e.steem_balance >= o.steem_amount,
                    "Release amount exceeds escrow balance. Amount: ${a}, Balance: ${b}", ( "a", o.steem_amount ) ( "b",
                        e.steem_balance ) );
        FC_ASSERT ( e.sbd_balance >= o.sbd_amount,
                    "Release amount exceeds escrow balance. Amount: ${a}, Balance: ${b}", ( "a", o.sbd_amount ) ( "b",
                        e.sbd_balance ) );
        FC_ASSERT ( e.to == o.to, "Operation 'to' (${o}) does not match escrow 'to' (${e}).", ( "o",
                                                                                                o.to ) ( "e", e.to ) );
        FC_ASSERT ( e.agent == o.agent, "Operation 'agent' (${a}) does not match escrow 'agent' (${e}).",
        ...
        FC_ASSERT ( o.receiver == e.from ||
                    o.receiver == e.to, "Funds must be released to 'from' (${f}) or 'to' (${t})", ( "f", e.from ) ( "t",
                        e.to ) );
        FC_ASSERT ( e.to_approved &&
        ...
        if ( e.disputed ) {
          FC_ASSERT ( o.who == e.agent, "Only 'agent' (${a}) can release funds in a disputed escrow.", ( "a",
                                                                                                         e.agent ) );
        }

        else {
          FC_ASSERT ( o.who == e.from ||
                      o.who == e.to, "Only 'from' (${f}) and 'to' (${t}) can release funds from a non-disputed escrow",
                      ( "f", e.from ) ( "t", e.to ) );

          if ( e.escrow_expiration > _db.head_block_time() ) {
            // If there is no dispute and escrow has not expired, either party can release funds to the other.
            if ( o.who == e.from ) {
              FC_ASSERT ( o.receiver == e.to, "Only 'from' (${f}) can release funds to 'to' (${t}).", ( "f",
                                                                                                        e.from ) ( "t", e.to ) );
            }

            else
              if ( o.who == e.to ) {
                FC_ASSERT ( o.receiver == e.from, "Only 'to' (${t}) can release funds to 'from' (${t}).", ( "f",
                                                                                                            e.from ) ( "t", e.to ) );
              }
          }
        }
    ...
    void withdraw_vesting_evaluator::do_apply ( const withdraw_vesting_operation& o )
    {
      ...
      if ( o.vesting_shares.amount == 0 ) {
        ...
        //! at calibrae genesis has_hardfork always evaluates true
        //! this assignment could be skipped but also this will be rewritten
        //! beccause calibrae's power down just calculates remaining divided by 100
        //! possibly the percentage could be changed to 1/128 and allow a 7 place bitshift operation
        //! which would be a lot faster to calculate, but not so easy in decimal notation
        //! the decision which will be based on how often this calculation must be done, since it is
        //! daily, it may be more optimal to use the fraction rather than the decimal
        ...
        _db.modify ( account, [&] ( account_object & a ) {
          ...
          a.next_vesting_withdrawal = _db.head_block_time() + fc::seconds (
    ...
    void set_withdraw_vesting_route_evaluator::do_apply ( const set_withdraw_vesting_route_operation&
                                                          o )
    {
      try {
        ...
        const auto& wd_idx =
          _db.get_index< withdraw_vesting_route_index >().indices().get< by_withdraw_route >();
      ...
      try {
        ...
        if (
          ...
          const auto& comment_vote_idx =
            _db.get_index< comment_vote_index >().indices().get< by_comment_voter >();
        ...
        int64_t abs_rshares    = ( ( uint128_t ( voter.effective_vesting_shares().amount.value ) *
        ...
        FC_ASSERT ( abs_rshares > STEEMIT_VOTE_DUST_THRESHOLD ||
                    o.weight == 0, "Voting weight is too small, please accumulate more voting power or steem power." );
        // }

        // else
        //   if ( _db.has_hardfork ( STEEMIT_HARDFORK_0_13__248 ) ) {
        //     FC_ASSERT ( abs_rshares > STEEMIT_VOTE_DUST_THRESHOLD ||
        //                 abs_rshares == 1,
        ...
        if ( itr == comment_vote_idx.end() ) {
          ...
          //! the following variable only is used when hardfork logic is present
          ...
          //     avg_cashout_sec = ( cur_cashout_time_sec * old_root_abs_rshares + new_cashout_time_sec *
          //                         abs_rshares ) / ( old_root_abs_rshares + abs_rshares );
          //   }
          ...
          **/
          _db.create<comment_vote_object> ( [&] ( comment_vote_object & cv ) {
            ...
            if ( curation_reward_eligible
                 //! at calibrae genesis has_hardfork always evaluates true
                 //  && _db.has_hardfork ( STEEMIT_HARDFORK_0_17__774 )
               ) {
            ...
            if ( curation_reward_eligible ) {
              if ( comment.created < fc::time_point_sec ( STEEMIT_HARDFORK_0_6_REVERSE_AUCTION_TIME ) ) {
                ...
                //! This variable is not used with HF logic removed
                ...
                // else
                //   if ( _db.has_hardfork ( STEEMIT_HARDFORK_0_1 ) ) {
                //     uint64_t old_weight = ( ( std::numeric_limits< uint64_t >::max() * fc::uint128_t (
                //                                 old_vote_rshares.value ) ) / ( two_s + old_vote_rshares.value ) ).to_uint64();
                //     uint64_t new_weight = ( ( std::numeric_limits< uint64_t >::max() * fc::uint128_t (
                //                                 comment.vote_rshares.value ) ) / ( two_s + comment.vote_rshares.value ) ).to_uint64();
                //     cv.weight = new_weight - old_weight;
                //   }
                //   else {
                //     uint64_t old_weight = ( ( std::numeric_limits< uint64_t >::max() * fc::uint128_t (
                //                                 1000000 * old_vote_rshares.value ) ) / ( two_s + ( 1000000 *
                //                                                                                    old_vote_rshares.value ) ) ).to_uint64();
                //     uint64_t new_weight = ( ( std::numeric_limits< uint64_t >::max() * fc::uint128_t (
                //                                 1000000 * comment.vote_rshares.value ) ) / ( two_s + ( 1000000 *
          ...
          if ( itr->rshares < rshares ) {
            ...
            // else
            //   if ( _db.has_hardfork ( STEEMIT_HARDFORK_0_7 ) ) {
            //     FC_ASSERT ( _db.head_block_time() < _db.calculate_discussion_payout_time (
            //                   comment ) - STEEMIT_UPVOTE_LOCKOUT_HF7,
    ...
    void custom_binary_evaluator::do_apply ( const custom_binary_operation& o )
    {
      database& d = db();
      //! at calibrae genesis has_hardfork always evaluates true
      //! This function only exists after HF14 so it remains in place for calibrae genesis
    ...
    void pow_apply ( database& db, Operation o )
    {
      ...
      if ( itr == accounts_by_name.end() ) {
        db.create< account_object > ( [&] ( account_object & acc ) {
          ...
          // { acc.recovery_account = "steem"; } else {
    ...
    void pow_evaluator::do_apply ( const pow_operation& o )
    {
      ...
      // "pow is deprecated. Use pow2 instead" );
      pow_apply ( db(), o );
    }
    ...
    {
      FC_ASSERT ( o.expiration > _db.head_block_time(),
    ...
    {
      //! at calibrae genesis has_hardfork always evaluates true
      // FC_ASSERT ( !_db.has_hardfork ( STEEMIT_HARDFORK_0_4 ),
    ...
    void challenge_authority_evaluator::do_apply ( const challenge_authority_operation& o )
    {
      ...
      if ( o.require_owner ) {
        ...
        FC_ASSERT ( _db.head_block_time() - challenged.last_active_proved >
    ...
    void request_account_recovery_evaluator::do_apply ( const request_account_recovery_operation& o )
    {
      ...
      if ( request == recovery_request_idx.end() ) { // New Request
        ...
        if ( o.new_owner_authority.weight_threshold == 0 ) { // Cancel Request if authority is open
          _db.remove ( *request );
        }
    ...
    void recover_account_evaluator::do_apply ( const recover_account_operation& o )
    {
      ...
      const auto& recent_auth_idx =
        _db.get_index< owner_authority_history_index >().indices().get< by_account >();
    ...
    void change_recovery_account_evaluator::do_apply ( const change_recovery_account_operation& o )
    {
      ...
      if ( request == change_recovery_idx.end() ) { // New request
        _db.create< change_recovery_account_request_object > ( [&] (
        ...
        if ( account_to_recover.recovery_account !=
    ...
    void cancel_transfer_from_savings_evaluator::do_apply ( const cancel_transfer_from_savings_operation&
                                                            op )
    {
    ...
    void decline_voting_rights_evaluator::do_apply ( const decline_voting_rights_operation& o )
    {
      ...
      const auto& request_idx =
        _db.get_index< decline_voting_rights_request_index >().indices().get< by_account >();
      auto itr = request_idx.find ( account.id );

      if ( o.decline ) {
        FC_ASSERT ( itr == request_idx.end(), "Cannot create new request because one already exists." );
        _db.create< decline_voting_rights_request_object > ( [&] ( decline_voting_rights_request_object &
        req ) {
    ...
    void claim_reward_balance_evaluator::do_apply ( const claim_reward_balance_operation& op )
    {
      ...
      FC_ASSERT ( op.reward_sbd <= acnt.reward_sbd_balance,
                  "Cannot claim that much SBD. Claim: ${c} Actual: ${a}",
      ...
      { reward_vesting_steem_to_move = acnt.reward_vesting_steem; }
        ...
        reward_vesting_steem_to_move = asset ( ( ( uint128_t ( op.reward_vests.amount.value ) * uint128_t (
                                                     acnt.reward_vesting_steem.amount.value ) )
    ...
    void delegate_vesting_shares_evaluator::do_apply ( const delegate_vesting_shares_operation& op )
    {
      ...
      auto delegation = _db.find< vesting_delegation_object, by_delegation > ( boost::make_tuple (
                                                                                 op.delegator, op.delegatee ) );
      auto available_shares = delegator.vesting_shares - delegator.delegated_vesting_shares - asset (
                                delegator.to_withdraw - delegator.withdrawn, VESTS_SYMBOL );
      const auto& wso = _db.get_witness_schedule_object();
      const auto& gpo = _db.get_dynamic_global_properties();
      auto min_delegation = asset ( wso.median_props.account_creation_fee.amount * 10,
                                    STEEM_SYMBOL ) * gpo.get_vesting_share_price();
      ...
      if ( delegation == nullptr ) {
        ...
        FC_ASSERT ( op.vesting_shares >= min_delegation, "Account must delegate a minimum of ${v}", ( "v",
                                                                                                      min_delegation ) );
      ...
      // Else if the delegation is increasing
        ...
        if ( op.vesting_shares >= delegation->vesting_shares ) {
          ...
          FC_ASSERT ( delta >= min_update,
                      "Steem Power increase is not enough of a difference. min_update: ${min}", ( "min", min_update ) );
        ...
        // Else the delegation is decreasing
          ...
          if ( op.vesting_shares.amount > 0 ) {
            FC_ASSERT ( delta >= min_update,
                        "Steem Power decrease is not enough of a difference. min_update: ${min}", ( "min", min_update ) );
          ...
          _db.create< vesting_delegation_expiration_object > ( [&] ( vesting_delegation_expiration_object &
          obj ) {
            obj.delegator = op.delegator;
