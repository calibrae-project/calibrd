...
namespace steemit {
  namespace chain {
    ...
    void update_median_witness_props ( database& db )
    {
      ...
      std::sort ( active.begin(), active.end(), [&] ( const witness_object * a,
    ...
    void update_witness_schedule4 ( database& db )
    {
      ...
      for ( auto itr = widx.begin();
        ...
        if (
          ...
          // db.has_hardfork( STEEMIT_HARDFORK_0_14__278 ) &&
      ...
      //! All references to mining need to be removed from this algorithm
      ...
      for ( auto witness_count = selected_voted.size();
        ...
        if (
          // selected_miners.find(sitr->id) == selected_miners.end() &&
      ...
      auto num_timeshare = active_witnesses.size();
      //  - num_miners - num_elected;
      ...
      for ( auto itr = processed_witnesses.begin(); itr != processed_witnesses.end(); ++itr ) {
        auto new_virtual_scheduled_time = new_virtual_time + VIRTUAL_SCHEDULE_LAP_LENGTH2 / ( (
                                                                                                *itr )->votes.value + 1 );
      ...
      FC_ASSERT ( active_witnesses.size() == expected_active_witnesses,
                  ...
                  ( "active_witnesses.size()", active_witnesses.size() ) ( "STEEMIT_MAX_WITNESSES",
                                                                           STEEMIT_MAX_WITNESSES ) ( "expected_active_witnesses", expected_active_witnesses ) );
    ...
    void update_witness_schedule ( database& db )
    {
      if ( ( db.head_block_num() % STEEMIT_MAX_WITNESSES ) == 0 ) { //wso.next_shuffle_block_num )
        ...
        return;
        //! Everything from here on in this if block will not execute due to the return above
        // }
        ...
        //         (active_witnesses.size() < (STEEMIT_MAX_WITNESSES - 2)); ++itr ) {
        //     if ( itr->pow_worker )
        //     { continue; }
        //     active_witnesses.push_back(itr->owner);
