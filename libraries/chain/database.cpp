...
namespace steemit {
  namespace chain {
    ...
    void database::open ( const fc::path& data_dir, const fc::path& shared_mem_dir,
                          uint64_t initial_supply, uint64_t shared_file_size,
      ...
      try {
        ...
        if ( chainbase_flags & chainbase::database::read_write ) {
          if ( !find< dynamic_global_property_object > ( ) )
            with_write_lock ( [&]( ) { init_genesis ( initial_supply ); } );
          _block_log.open ( data_dir / "block_log" );
          auto log_head = _block_log.head ( );
          // Rewind all undo state. This should return us to the state at the last irreversible
          // block.
          with_write_lock ( [&]( ) {
            undo_all ( );
    ...
    void database::reindex ( const fc::path& data_dir, const fc::path& shared_mem_dir,
                             uint64_t shared_file_size )
    {
      try {
        ...
        uint64_t skip_flags = skip_witness_signature | skip_transaction_signatures |
                              skip_transaction_dupe_check | skip_tapos_check | skip_merkle_check |
                              skip_witness_schedule_check | skip_authority_check |
    ...
    void database::wipe ( const fc::path& data_dir, const fc::path& shared_mem_dir,
                          bool include_blocks )
    {
      close ( );
    ...
    database::get_recent_transaction ( const transaction_id_type& trx_id ) const
    {
      try {
    ...
    database::get_block_ids_on_fork ( block_id_type head_of_fork ) const
    {
      try {
    ...
    const escrow_object& database::get_escrow ( const account_name_type& name,
    ...
    const escrow_object* database::find_escrow ( const account_name_type& name,
                                                 uint32_t escrow_id ) const
    {
    ...
    {
      try {
        //! at calibrae genesis has_hardfork always evaluates true
    ...
    database::calculate_discussion_payout_time ( const comment_object& comment ) const
    {
      //! at calibrae genesis has_hardfork always evaluates true
      //! with an OR condition this statement always evaluates true
    ...
    bool database::push_block ( const signed_block& new_block, uint32_t skip )
    {
      ...
      //   ilog( "push_block ${b} took ${t} microseconds", ("b", new_block.block_num())("t",
      //   dt.count()) );
      return result;
    }
    ...
    bool database::_push_block ( const signed_block& new_block )
    {
      try {
        ...
        if ( ! ( skip & skip_fork_db ) ) {
          ...
          // If the head block from the longest chain does not build off of the current head, we
          // need to switch forks.
          if ( new_head->data.previous != head_block_id ( ) ) {
            ...
            if ( new_head->data.block_num ( ) > head_block_num ( ) ) {
              ...
              for ( auto ritr = branches.first.rbegin ( ); ritr != branches.first.rend ( );
                    ++ritr ) {
                ...
                if ( except ) {
                  ...
                  for ( auto ritr = branches.second.rbegin ( ); ritr != branches.second.rend ( );
                        ++ritr ) {
                    auto session = start_undo_session ( true );
                    apply_block ( ( *ritr )->data, skip );
                    session.push ( );
     ...
     *
     * When called to push a locally generated transaction, set the skip_block_size_check bit on the
     * skip argument. This
     * will allow the transaction to be pushed even if it causes the pending block size to exceed
     * the maximum block size.
     * Although the transaction will probably not propagate further now, as the peers are likely to
     * have their pending
     * queues full as well, it will be kept in the queue to be propagated later when a new block
     * flushes out the pending
     * queues.
     */
      ...
      try {
        try {
          FC_ASSERT ( fc::raw::pack_size ( trx ) <=
                      ( get_dynamic_global_properties ( ).maximum_block_size - 256 ) );
          set_producing ( true );
          detail::with_skip_flags (
          *this, skip, [&]( ) { with_write_lock ( [&]( ) { _push_transaction ( trx ); } ); } );
          set_producing ( false );
        }
    ...
    void database::_push_transaction ( const signed_transaction& trx )
    {
      ...
      // This allows us to quickly rewind to the clean state of the head block, in case a new block
      // arrives.
    ...
    signed_block
    database::_generate_block ( fc::time_point_sec when, const account_name_type& witness_owner,
                                const fc::ecc::private_key& block_signing_private_key )
    {
      uint32_t skip = get_node_properties ( ).skip_flags;
      ...
      auto maximum_block_size =
        get_dynamic_global_properties ( ).maximum_block_size; // STEEMIT_MAX_BLOCK_SIZE;
      ...
      if ( has_hardfork ( STEEMIT_HARDFORK_0_5__54 ) ) {
        ...
        if ( witness.running_version != STEEMIT_BLOCKCHAIN_VERSION )
          pending_block.extensions.insert (
            block_header_extensions ( STEEMIT_BLOCKCHAIN_VERSION ) );

        const auto& hfp = get_hardfork_property_object ( );

        if ( hfp.current_hardfork_version < STEEMIT_BLOCKCHAIN_HARDFORK_VERSION // Binary is newer
             // hardfork than has
             // been applied
          ...
          if ( hfp.current_hardfork_version ==
               STEEMIT_BLOCKCHAIN_HARDFORK_VERSION // Binary does not know of a new
               // hardfork
               &&
               witness.hardfork_version_vote >
               STEEMIT_BLOCKCHAIN_HARDFORK_VERSION ) // Voting for hardfork in the future,
            // that we do not know of...
          {
            // Make vote match binary configuration. This is vote to not apply the new hardfork.
            pending_block.extensions.insert ( block_header_extensions ( hardfork_version_vote (
                                                                          _hardfork_versions[hfp.last_hardfork], _hardfork_times[hfp.last_hardfork] ) ) );
          }
      }
                                     ...
                                     bool to_reward_balance )
    {
      try {
         ...
         *
         *  128 bit math is requred due to multiplying of 64 bit numbers. This is done in asset and
         * price.
         */
    ...
    void database::adjust_proxied_witness_votes (
      ...
      const std::array < share_type, STEEMIT_MAX_PROXY_RECURSION_DEPTH + 1 > &delta, int depth )
    {
    ...
    void database::adjust_proxied_witness_votes ( const account_object& a, share_type delta,
                                                  int depth )
    {
      if ( a.proxy != STEEMIT_PROXY_TO_SELF_ACCOUNT ) {
        /// nested proxies are not supported, vote will not propagate
    ...
    {
      const auto& vidx =
        get_index< witness_vote_index > ( ).indices ( ).get< by_account_witness > ( );
    ...
    void database::adjust_witness_vote ( const witness_object& witness, share_type delta )
    {
      ...
      modify ( witness, [&] ( witness_object & w ) {
        ...
        FC_ASSERT ( w.votes <= get_dynamic_global_properties ( ).total_vesting_shares.amount, "",
                    ( "w.votes", w.votes ) (
                      "props", get_dynamic_global_properties ( ).total_vesting_shares ) );

        if ( has_hardfork ( STEEMIT_HARDFORK_0_2 ) )
          w.virtual_scheduled_time =
            w.virtual_last_update +
            ( VIRTUAL_SCHEDULE_LAP_LENGTH2 - w.virtual_position ) / ( w.votes.value + 1 );
        ...
        /** witnesses with a low number of votes could overflow the time field and end up with a
    ...
    /**
     * This method updates total_reward_shares2 on DGPO, and children_rshares2 on comments, when a
     * comment's rshares2 changes
     * from old_rshares2 to new_rshares2.  Maintaining invariants that children_rshares2 is the sum
     * of all descendants' rshares2,
    ...
    void database::update_owner_authority ( const account_object& account,
                                            const authority& owner_authority )
    {
      if ( head_block_num ( ) >= STEEMIT_OWNER_AUTH_HISTORY_TRACKING_START_BLOCK_NUM ) {
        create< owner_authority_history_object > ( [&] ( owner_authority_history_object & hist ) {
          hist.account = account.name;
          hist.previous_owner_authority =
            get< account_authority_object, by_account > ( account.name ).owner;
    ...
    {
      const auto& widx =
        get_index< account_index > ( ).indices ( ).get< by_next_vesting_withdrawal > ( );
      const auto& didx =
        get_index< withdraw_vesting_route_index > ( ).indices ( ).get< by_withdraw_route > ( );
      ...
      while ( current != widx.end ( ) && current->next_vesting_withdrawal <= head_block_time ( ) ) {
        ...
        */
        ...
        if ( from_account.to_withdraw - from_account.withdrawn <
          ...
          to_withdraw =
            std::min ( from_account.vesting_shares.amount,
        ...
        // Do two passes, the first for vests, the second for steem. Try to maintain as much
        // accuracy for vests as possible.
        for ( auto itr =
                didx.upper_bound ( boost::make_tuple ( from_account.id, account_id_type ( ) ) );
              itr != didx.end ( ) && itr->from_account == from_account.id; ++itr ) {
          if ( itr->auto_vest ) {
            share_type to_deposit =
              ( ( fc::uint128_t ( to_withdraw.value ) * itr->percent ) / STEEMIT_100_PERCENT )
              .to_uint64 ( );
            ...
            if ( to_deposit > 0 ) {
              ...
              modify ( to_account,
              [&] ( account_object & a ) { a.vesting_shares.amount += to_deposit; } );
              adjust_proxied_witness_votes ( to_account, to_deposit );
              push_virtual_operation ( fill_vesting_withdraw_operation (
                                         from_account.name, to_account.name, asset ( to_deposit, VESTS_SYMBOL ),
        ...
        for ( auto itr =
          ...
          if ( !itr->auto_vest ) {
            ...
            if ( to_deposit > 0 ) {
              modify ( to_account, [&] ( account_object & a ) { a.balance += converted_steem; } );
        ...
        auto converted_steem =
          asset ( to_convert, VESTS_SYMBOL ) * cprops.get_vesting_share_price ( );
        ...
        push_virtual_operation ( fill_vesting_withdraw_operation (
                                   from_account.name, from_account.name, asset ( to_withdraw, VESTS_SYMBOL ),
                                   converted_steem ) );
      }
    }
      ...
      try {
        ...
        if ( !c.allow_curation_rewards ) {
          ...
          if ( c.total_vote_weight > 0 ) {
            const auto& cvidx =
              get_index< comment_vote_index > ( ).indices ( ).get< by_comment_weight_voter > ( );
            ...
            while ( itr != cvidx.end ( ) && itr->comment == c.id ) {
              ...
              if ( claim > 0 ) { // min_amt is non-zero satoshis
                ...
                push_virtual_operation ( curation_reward_operation ( voter.name, reward, c.author,
    ...
    void fill_comment_reward_context_local_state ( util::comment_reward_context& ctx,
    ...
    share_type database::cashout_comment_helper ( util::comment_reward_context& ctx,
                                                  const comment_object& comment )
    {
      try {
        ...
        if ( comment.net_rshares > 0 ) {
          ...
          if ( reward_tokens > 0 ) {
            share_type curation_tokens =
              ( ( reward_tokens * get_curation_rewards_percent ( comment ) ) /
                STEEMIT_100_PERCENT )
              .to_uint64 ( );
            ...
            auto sbd_steem =
              ( author_tokens * comment.percent_steem_dollars ) / ( 2 * STEEMIT_100_PERCENT );
            auto vesting_steem = author_tokens - sbd_steem;
            const auto& author = get_account ( comment.author );
            auto vest_created = create_vesting ( author, vesting_steem,
                                                 has_hardfork ( STEEMIT_HARDFORK_0_17__659 ) );
            auto sbd_payout =
              create_sbd ( author, sbd_steem, has_hardfork ( STEEMIT_HARDFORK_0_17__659 ) );
            adjust_total_payout (
              comment,
              sbd_payout.first +
            ...
            [&] ( account_object & a ) { a.posting_rewards += author_tokens; } );
            #endif
          }

          if ( !has_hardfork ( STEEMIT_HARDFORK_0_17__774 ) )
            adjust_rshares2 ( comment, util::evaluate_reward_curve ( comment.net_rshares.value ),
                              0 );
        }
        ...
        const auto& vote_idx =
          get_index< comment_vote_index > ( ).indices ( ).get< by_comment_voter > ( );
    ...
    void database::process_comment_cashout ( )
    {
      ...
      /// and people have had a week to start posting.  The first cashout will be the biggest
      /// because it
      ...
      for ( auto itr = reward_idx.begin ( ); itr != reward_idx.end ( ); ++itr ) {
        ...
        modify ( *itr, [&] ( reward_fund_object & rfo ) {
          ...
          rfo.recent_claims -=
            ( rfo.recent_claims * ( head_block_time ( ) - rfo.last_update ).to_seconds ( ) ) /
      ...
      //  add all rshares about to be cashed out to the reward funds. This ensures equal satoshi per
      //  rshare payment
        ...
        while ( current != cidx.end ( ) && current->cashout_time <= head_block_time ( ) ) {
          if ( current->net_rshares > 0 ) {
            ...
            funds[rf.id._id].recent_claims += util::evaluate_reward_curve (
                                                current->net_rshares.value, rf.author_reward_curve, rf.content_constant );
          }
      ...
      if ( funds.size ( ) ) {
        for ( size_t i = 0; i < funds.size ( ); i++ ) {
          modify ( get< reward_fund_object, by_id > ( reward_fund_id_type ( i ) ),
     ...
     *
     *  This method pays out vesting and reward shares every block, and liquidity shares once per
     * day.
    ...
    void database::process_funds ( )
    {
      ...
      if ( has_hardfork ( STEEMIT_HARDFORK_0_16__551 ) ) {
        /**
         * At block 7,000,000 have a 9.5% instantaneous inflation rate, decreasing to 0.95% at a
         * rate of 0.01%
         * every 250k blocks. This narrowing will take approximately 20.5 years and will complete on
         * block 220,750,000
         */
        ...
        int64_t current_inflation_rate =
          std::max ( start_inflation_rate - inflation_rate_adjustment, inflation_rate_floor );
        ...
        auto vesting_reward = ( new_steem * STEEMIT_VESTING_FUND_PERCENT ) /
                              STEEMIT_100_PERCENT; /// 15% to vesting fund
        auto witness_reward =
          new_steem - content_reward - vesting_reward; /// Remaining 10% to witness pay
        ...
        { witness_reward *= wso.timeshare_weight; }
          ...
          if ( cwit.schedule == witness_object::miner )
          { witness_reward *= wso.miner_weight; }

          else
            if ( cwit.schedule == witness_object::top19 )
    ...
    {
      const auto& idx =
        get_index< savings_withdraw_index > ( ).indices ( ).get< by_complete_from_rid > ( );
      auto itr = idx.begin ( );
      ...
      while ( itr != idx.end ( ) ) {
        ...
        modify ( get_account ( itr->from ),
        [&] ( account_object & a ) { a.savings_withdraw_requests--; } );
    ...
    asset database::get_liquidity_reward ( ) const
    {
      ...
      asset percent ( protocol::calc_percent_reward_per_hour< STEEMIT_LIQUIDITY_APR_PERCENT > (
    ...
    asset database::get_producer_reward ( )
    {
      const auto& props = get_dynamic_global_properties ( );
      static_assert ( STEEMIT_BLOCK_INTERVAL == 3, "this code assumes a 3-second time interval" );
      asset percent ( protocol::calc_percent_reward_per_block< STEEMIT_PRODUCER_APR_PERCENT > (
                        props.virtual_supply.amount ),
                      STEEM_SYMBOL );
      ...
      if ( props.head_block_number >= STEEMIT_START_MINER_VOTING_BLOCK ||
        ...
        push_virtual_operation (
          producer_reward_operation ( witness_account.name, producer_reward ) );
      }

      else {
        modify ( get_account ( witness_account.name ),
        [&] ( account_object & a ) { a.balance += pay; } );
      }
    ...
    void database::pay_liquidity_reward ( )
    {
      ...
      if ( ( head_block_num ( ) % STEEMIT_LIQUIDITY_REWARD_BLOCKS ) == 0 ) {
        ...
        const auto& ridx =
          get_index< liquidity_reward_balance_index > ( ).indices ( ).get< by_volume_weight > ( );
        auto itr = ridx.begin ( );
    ...
    share_type database::pay_reward_funds ( share_type reward )
    {
      ...
      for ( auto itr = reward_idx.begin ( ); itr != reward_idx.end ( ); ++itr ) {
        ...
        used_rewards += r;
        // Sanity check to ensure we aren't printing more STEEM than has been allocated through
        // inflation
    ...
    void database::process_conversions ( )
    {
      ...
      const auto& request_by_date =
        get_index< convert_request_index > ( ).indices ( ).get< by_conversion_date > ( );
      ...
      while ( itr != request_by_date.end ( ) && itr->conversion_date <= now ) {
        ...
        push_virtual_operation ( fill_convert_request_operation ( user.name, itr->requestid,
                                                                  itr->amount, amount_to_issue ) );
        remove ( *itr );
    ...
    void database::account_recovery_processing ( )
    {
      ...
      const auto& rec_req_idx =
        get_index< account_recovery_request_index > ( ).indices ( ).get< by_expiration > ( );
      ...
      const auto& change_req_idx = get_index< change_recovery_account_request_index > ( )
                                   .indices ( )
                                   .get< by_effective_date > ( );
    ...
    {
      const auto& escrow_idx =
        get_index< escrow_index > ( ).indices ( ).get< by_ratification_deadline > ( );
    ...
    {
      const auto& request_idx = get_index< decline_voting_rights_request_index > ( )
                                .indices ( )
                                .get< by_effective_date > ( );
      ...
      const std::string& id, std::shared_ptr< custom_operation_interpreter > registry )
    {
      ...
      // This assert triggering means we're mis-configured (multiple registrations of custom JSON
      // evaluator for same ID)
      FC_ASSERT ( inserted );
    }

    std::shared_ptr< custom_operation_interpreter >
    database::get_custom_json_evaluator ( const std::string& id )
    {
    ...
    void database::init_schema ( )
    {
      ...
      for( std::shared_ptr< abstract_schema >& s : schema_list )
      {
         std::string tname;
         s->get_name( tname );
         FC_ASSERT( ds.types.find( tname ) == ds.types.end(), "types with different ID's found for
      ...
      try {
        struct auth_inhibitor {
            auth_inhibitor ( database& db )
              : db ( db ), old_flags ( db.node_properties ( ).skip_flags ) {
        ...
        create< account_object > ( [&] ( account_object & a ) { a.name = STEEMIT_MINER_ACCOUNT; } );
        create< account_authority_object > ( [&] ( account_authority_object & auth ) {
          auth.account = STEEMIT_MINER_ACCOUNT;
          auth.owner.weight_threshold = 1;
          auth.active.weight_threshold = 1;
        } );
        create< account_object > ( [&] ( account_object & a ) { a.name = STEEMIT_NULL_ACCOUNT; } );
        create< account_authority_object > ( [&] ( account_authority_object & auth ) {
          auth.account = STEEMIT_NULL_ACCOUNT;
          auth.owner.weight_threshold = 1;
          auth.active.weight_threshold = 1;
        } );
        create< account_object > ( [&] ( account_object & a ) { a.name = STEEMIT_TEMP_ACCOUNT; } );
        create< account_authority_object > ( [&] ( account_authority_object & auth ) {
      ...
      try {
        ...
        if ( _checkpoints.size ( ) && _checkpoints.rbegin ( )->second != block_id_type ( ) ) {
          ...
          if ( _checkpoints.rbegin ( )->first >= block_num )
            skip = skip_witness_signature | skip_transaction_signatures |
                   skip_transaction_dupe_check | skip_fork_db | skip_block_size_check |
                   skip_tapos_check | skip_authority_check
                   /* | skip_merkle_check While blockchain is being downloaded, txs need to be
                      validated against block headers */
    ...
    void database::show_free_memory ( bool force )
    {
      ...
      if ( force || ( free_gb < _last_free_gb_printed ) ||
      ...
      if ( free_gb == 0 ) {
        ...
        if ( free_mb <= 100 && head_block_num ( ) % 10 == 0 )
          elog ( "Free memory is now ${n}M. Increase shared file size immediately!",
                 ( "n", free_mb ) );
      }
    }
      ...
      try {
          ...
          try {
            FC_ASSERT ( next_block.transaction_merkle_root == merkle_root, "Merkle check failed",
                        ( "next_block.transaction_merkle_root",
                          next_block.transaction_merkle_root ) ( "calc", merkle_root ) (
        ...
        if ( has_hardfork ( STEEMIT_HARDFORK_0_12 ) ) {
          FC_ASSERT ( block_size <= gprops.maximum_block_size, "Block Size is too Big",
                      ( "next_block_num", next_block_num ) ( "block_size", block_size ) (
        ...
        if ( has_hardfork ( STEEMIT_HARDFORK_0_5__54 ) ) { // Cannot remove after hardfork
          ...
          FC_ASSERT ( witness.running_version >= hardfork_state.current_hardfork_version,
                      ...
                      ( "witness", witness ) ( "next_block.witness", next_block.witness ) (
        ...
        switch ( itr->which ( ) ) {
          ...
          case 1: { // version
              ...
              if ( reported_version != signing_witness.running_version ) {
                modify ( signing_witness,
                [&] ( witness_object & wo ) { wo.running_version = reported_version; } );
              }
      ...
      try {
        ...
        for ( int i = 0; i < wso.num_scheduled_witnesses; i++ ) {
          ...
          if ( has_hardfork ( STEEMIT_HARDFORK_0_19__822 ) ) {
            ...
            if ( wit.last_sbd_exchange_update < now + STEEMIT_MAX_FEED_AGE_SECONDS &&
                 !wit.sbd_exchange_rate.is_null ( ) ) {
              feeds.push_back ( wit.sbd_exchange_rate );
            }
        }
    ...
    void database::_apply_transaction ( const signed_transaction& trx )
    {
      try {
        ...
        // Skip all manner of expiration and TaPoS checking if we're on block 1; It's impossible
        // that the transaction is
        // expired, and TaPoS makes no sense as no blocks exist.
        if ( BOOST_LIKELY ( head_block_num ( ) > 0 ) ) {
          if ( ! ( skip & skip_tapos_check ) ) {
            const auto& tapos_block_summary = get< block_summary_object > ( trx.ref_block_num );
            // Verify TaPoS block summary has correct ID prefix, and that this block's time is not
            // past the expiration
          ...
          STEEMIT_ASSERT ( trx.expiration <=
                           now + fc::seconds ( STEEMIT_MAX_TIME_UNTIL_EXPIRATION ),
                           transaction_expiration_exception, "",
                           ( "trx.expiration", trx.expiration ) ( "now", now ) (
                             "max_til_exp", STEEMIT_MAX_TIME_UNTIL_EXPIRATION ) );

          if ( has_hardfork ( STEEMIT_HARDFORK_0_9 ) ) // Simple solution to pending trx bug when
            // now == trx.expiration
    ...
    const witness_object& database::validate_block_header ( uint32_t skip,
      ...
      try {
        ...
        if ( ! ( skip & skip_witness_schedule_check ) ) {
          ...
          FC_ASSERT ( witness.owner == scheduled_witness, "Witness produced block at wrong time",
                      ( "block witness", next_block.witness ) ( "scheduled", scheduled_witness ) (
                        "slot_num", slot_num ) );
        }
    ...
    void database::update_global_dynamic_data ( const signed_block& b )
    {
      try {
        ...
        modify ( _dgp, [&] ( dynamic_global_property_object & dgp ) {
          // This is constant time assuming 100% participation. It is O(B) otherwise (B = Num blocks
          // between update)
        ...
        if ( ! ( get_node_properties ( ).skip_flags & skip_undo_history_check ) ) {
          STEEMIT_ASSERT (
            ...
            undo_database_exception, "The database does not have enough undo history to support "
            "a blockchain with so many missed blocks. "
            "Please add a checkpoint if you would like to continue "
            "applying blocks beyond this point.",
            ( "last_irreversible_block_num", _dgp.last_irreversible_block_num ) (
              "head", _dgp.head_block_number ) ( "max_undo", STEEMIT_MAX_UNDO_HISTORY ) );
        }
      }
      ...
      try {
        modify ( get_dynamic_global_properties ( ), [&] ( dynamic_global_property_object & dgp ) {
          ...
          if ( !median_price.is_null ( ) && has_hardfork ( STEEMIT_HARDFORK_0_14__230 ) ) {
            auto percent_sbd =
              uint16_t ( ( ( fc::uint128_t ( ( dgp.current_sbd_supply *
            ...
            { dgp.sbd_print_rate = STEEMIT_100_PERCENT; }
              ...
              if ( percent_sbd >= STEEMIT_SBD_STOP_PERCENT )
              { dgp.sbd_print_rate = 0; }

              else
                dgp.sbd_print_rate =
                  ( ( STEEMIT_SBD_STOP_PERCENT - percent_sbd ) * STEEMIT_100_PERCENT ) /
    ...
    void database::update_signing_witness ( const witness_object& signing_witness,
        ...
        if ( ! ( get_node_properties ( ).skip_flags & skip_block_log ) ) {
          ...
          if ( log_head_num < dpo.last_irreversible_block_num ) {
            while ( log_head_num < dpo.last_irreversible_block_num ) {
              shared_ptr< fork_item > block =
                _fork_db.fetch_block_on_main_branch_by_number ( log_head_num + 1 );
              FC_ASSERT ( block, "Current fork in the fork database does not contain the "
    ...
    bool database::apply_order ( const limit_order_object& new_order_object )
    {
      ...
      const auto& limit_price_idx =
        get_index< limit_order_index > ( ).indices ( ).get< by_price > ( );
      ...
      while ( !finished && limit_itr != limit_end ) {
        ...
        ++limit_itr;
        // match returns 2 when only the old order was fully filled. In this case, we keep matching;
        // otherwise, we stop.
                          ...
                          const price& match_price )
    {
      ...
      if ( new_order_for_sale <= old_order_for_sale * match_price ) {
        ...
        // Although new_order_for_sale is greater than old_order_for_sale * match_price,
        // old_order_for_sale == new_order_for_sale * match_price
      ...
      push_virtual_operation ( fill_order_operation ( new_order.seller, new_order.orderid,
    ...
    void database::adjust_liquidity_reward ( const account_object& owner, const asset& volume,
                                             bool is_sdb )
    {
      const auto& ridx =
        get_index< liquidity_reward_balance_index > ( ).indices ( ).get< by_owner > ( );
      auto itr = ridx.find ( owner.id );

      if ( itr != ridx.end ( ) ) {
        modify< liquidity_reward_balance_object > (
        *itr, [&] ( liquidity_reward_balance_object & r ) {
    ...
    bool database::fill_order ( const limit_order_object& order, const asset& pays,
    ...
    {
      auto now = head_block_time ( );
      const auto& orders_by_exp =
        get_index< limit_order_index > ( ).indices ( ).get< by_expiration > ( );
    ...
    {
      auto now = head_block_time ( );
      const auto& delegations_by_exp =
        get_index< vesting_delegation_expiration_index, by_expiration > ( );
          ...
          case SBD_SYMBOL:
            if ( a.sbd_seconds_last_update != head_block_time ( ) ) {
              ...
              if ( acnt.sbd_seconds > 0 &&
                   ( acnt.sbd_seconds_last_update - acnt.sbd_last_interest_payment )
                   .to_seconds ( ) > STEEMIT_SBD_INTEREST_COMPOUND_INTERVAL_SEC ) {
          ...
          case SBD_SYMBOL:
            if ( a.savings_sbd_seconds_last_update != head_block_time ( ) ) {
              ...
              if ( acnt.savings_sbd_seconds > 0 &&
                   ( acnt.savings_sbd_seconds_last_update - acnt.savings_sbd_last_interest_payment )
                   .to_seconds ( ) > STEEMIT_SBD_INTEREST_COMPOUND_INTERVAL_SEC ) {
      ...
      modify ( props, [&] ( dynamic_global_property_object & props ) {
        switch ( delta.symbol ) {
          ...
          case SBD_SYMBOL:
            ...
            props.virtual_supply =
              props.current_sbd_supply * get_feed_history ( ).current_median_history +
    ...
    asset database::get_savings_balance ( const account_object& a,
    ...
    void database::init_hardforks ( )
    {
      ...
      FC_ASSERT ( hardforks.last_hardfork <= STEEMIT_NUM_HARDFORKS,
                  ...
                  ( "hardforks.last_hardfork",
                    hardforks.last_hardfork ) ( "STEEMIT_NUM_HARDFORKS", STEEMIT_NUM_HARDFORKS ) );
    ...
    void database::set_hardfork ( uint32_t hardfork, bool apply_now )
    {
      ...
      for ( uint32_t i = hardforks.last_hardfork + 1; i <= hardfork && i <= STEEMIT_NUM_HARDFORKS;
            i++ ) {
      ...
      switch ( hardfork ) {
        ...
        case STEEMIT_HARDFORK_0_9: {
            for ( const std::string& acc : hardfork9::get_compromised_accounts ( ) ) {
              ...
              update_owner_authority (
                *account,
                authority (
                  1, public_key_type ( "STM7sw22HqsXbz7D2CmJfmMwt9rimtk518dRzsR1f8Cgw52dQR1pR" ),
                  1 ) );
              modify (
                get< account_authority_object, by_account > ( account->name ),
              [&] ( account_authority_object & auth ) {
                auth.active = authority (
                                1,
                                public_key_type ( "STM7sw22HqsXbz7D2CmJfmMwt9rimtk518dRzsR1f8Cgw52dQR1pR" ),
                                1 );
        ...
        case STEEMIT_HARDFORK_0_12: {
            ...
            for ( auto itr = comment_idx.begin ( ); itr != comment_idx.end ( ); ++itr ) {
              // At the hardfork time, all new posts with no votes get their cashout time set to +12
              // hrs from head block time.
              // All posts with a payout get their cashout time set to +30 days. This hardfork takes
              // place within 30 days
              // initial payout so we don't have to handle the case of posts that should be frozen
              // that aren't
              if ( itr->parent_author == STEEMIT_ROOT_POST_PARENT ) {
                // Post has not been paid out and has no votes (cashout_time == 0 === net_rshares ==
                // 0, under current semmantics)
        ...
        case STEEMIT_HARDFORK_0_17: {
            static_assert ( STEEMIT_MAX_VOTED_WITNESSES_HF0 + STEEMIT_MAX_MINER_WITNESSES_HF0 +
            ...
            // The IDs must be assigned this way. The assertion is a dummy check to ensure this
            // happens.
            ...
            /*
            * For all current comments we will either keep their current cashout time, or extend it to
            * 1 week
            * after creation.
            *
            * We cannot do a simple iteration by cashout time because we are editting cashout time.
            * More specifically, we will be adding an explicit cashout time to all comments with
            * parents.
            * To find all discussions that have not been paid out we fir iterate over posts by cashout
            * time.
            * Before the hardfork these are all root posts. Iterate over all of their children, adding
            * each
            * to a specific list. Next, update payout times for all discussions on the root post. This
            * defines
            * the min cashout time for each child in the discussion. Then iterate over the children
            * and set
            * their cashout time in a similar way, grabbing the root post as their inherent cashout
            * time.
            */
            ...
            for ( auto itr = comment_idx.begin ( );
                  itr != comment_idx.end ( ) && itr->cashout_time < fc::time_point_sec::maximum ( );
                  ++itr ) {
              root_posts.push_back ( & ( *itr ) );

              for ( auto reply_itr = by_root_idx.lower_bound ( itr->id );
                    reply_itr != by_root_idx.end ( ) && reply_itr->root_comment == itr->id;
                    ++reply_itr ) {
            ...
            for ( auto itr : root_posts ) {
              modify ( *itr, [&] ( comment_object & c ) {
                c.cashout_time =
                  std::max ( c.created + STEEMIT_CASHOUT_WINDOW_SECONDS, c.cashout_time );
              } );
            }
    ...
    void database::validate_invariants ( ) const
    {
      try {
        ...
        for ( auto itr = account_idx.begin ( ); itr != account_idx.end ( ); ++itr ) {
          ...
          total_vsf_votes +=
            ( itr->proxy == STEEMIT_PROXY_TO_SELF_ACCOUNT
        ...
        for ( auto itr = convert_request_idx.begin ( ); itr != convert_request_idx.end ( );
              ++itr ) {
        ...
        for ( auto itr = limit_order_idx.begin ( ); itr != limit_order_idx.end ( ); ++itr ) {
          if ( itr->sell_price.base.symbol == STEEM_SYMBOL ) {
            ...
            if ( itr->sell_price.base.symbol == SBD_SYMBOL ) {
        ...
        for ( auto itr = escrow_idx.begin ( ); itr != escrow_idx.end ( ); ++itr ) {
          ...
          { total_supply += itr->pending_fee; }
            ...
            if ( itr->pending_fee.symbol == SBD_SYMBOL )
        ...
        const auto& savings_withdraw_idx =
          get_index< savings_withdraw_index > ( ).indices ( ).get< by_id > ( );

        for ( auto itr = savings_withdraw_idx.begin ( ); itr != savings_withdraw_idx.end ( );
              ++itr ) {
          if ( itr->amount.symbol == STEEM_SYMBOL )
          { total_supply += itr->amount; }
        ...
        total_supply += gpo.total_vesting_fund_steem + gpo.total_reward_fund_steem +
                        gpo.pending_rewarded_vesting_steem;
        ...
        FC_ASSERT ( gpo.total_vesting_shares + gpo.pending_rewarded_vesting_shares == total_vesting,
                    "", ( "gpo.total_vesting_shares",
                          gpo.total_vesting_shares ) ( "total_vesting", total_vesting ) );
        FC_ASSERT ( gpo.total_vesting_shares.amount == total_vsf_votes, "",
                    ( "total_vesting_shares", gpo.total_vesting_shares ) ( "total_vsf_votes",
                                                                           total_vsf_votes ) );
        ...
        if ( !get_feed_history ( ).current_median_history.is_null ( ) ) {
          FC_ASSERT ( gpo.current_sbd_supply * get_feed_history ( ).current_median_history +
                      gpo.current_supply ==
                      gpo.virtual_supply,
                      "", ( "gpo.current_sbd_supply", gpo.current_sbd_supply ) (
                        "get_feed_history().current_median_history",
                        get_feed_history ( ).current_median_history ) (
                        "gpo.current_supply", gpo.current_supply ) ( "gpo.virtual_supply",
                                                                     gpo.virtual_supply ) );
        }
      }
      ...
      try {
        ...
        for ( const auto& account : get_index< account_index > ( ).indices ( ) ) {
          modify ( account, [&] ( account_object & a ) {
            ...
            a.vesting_withdraw_rate = asset (
                                        a.to_withdraw / STEEMIT_VESTING_WITHDRAW_INTERVALS_PRE_HF_16, VESTS_SYMBOL );
    ...
    void database::retally_comment_children ( )
    {
      ...
      for ( auto itr = cidx.begin ( ); itr != cidx.end ( ); ++itr ) {
        if ( itr->parent_author != STEEMIT_ROOT_POST_PARENT ) {
          ...
          #ifdef IS_LOW_MEM
          modify ( get_comment ( itr->parent_author, itr->parent_permlink ),
          [&] ( comment_object & c ) { c.children++; } );
          #else
